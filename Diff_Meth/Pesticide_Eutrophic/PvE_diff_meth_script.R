...
write.table(subset_methBase_out, file="PvE_sites_for_PCA.txt", sep="\t", quote=F, row.names = F, col.names = T)
...
PCA_data1$Condition <- c(rep("Pesticide", 10), rep("Eutrophic", 9))
...
write.csv(diff_meth_filtered, file="diff_meth_PvE.csv") #edit for comparison; keep format